...
public class FavoriteLikeController {
	private final static String command = "/movieLike.tm";
	...
	public int doActionGet(
		...
		int cnt = -1;
		cnt = movieDao.MovieLikeUpdate(mnum);
		cnt = favoriteDao.MovieLikeInsert(favor);
		return cnt;
	}
	...
	public int doActionPost(
		...
		int cnt = -1;
		cnt = movieDao.MovieUnLikeUpdate(mnum);
		cnt = favoriteDao.MovieLikeDelete(favor);
		return cnt;
	}
